...
var escaped string
var unescaper, requoter, expander, sizer, duration1, duration2 *regexp.Regexp
var replacers [15]replacer
...
func (this *UConfig) Load(input string, inline ...bool) error {
	...
	for {
		...
		if start := strings.LastIndex(content[0:indexes[2]], "/*base:"); start >= 0 {
			if end := strings.Index(content[start:indexes[2]], "*/"); end >= start+7 {
				base = content[start+7 : end]
			}
		}
...
func (this *UConfig) GetBoolean(path string, fallback bool) bool {
	...
	if value = strings.ToLower(value); value == "1" || value == "on" || value == "yes" || value == "true" {
		return true
	}
	return false
}
...
func (this *UConfig) GetFloatBounds(path string, fallback, min, max float64) float64 {
	...
	nvalue, err := strconv.ParseFloat(value, 64)
	if err != nil {
		return fallback
	}
...
func (this *UConfig) GetDurationBounds(path string, fallback, min, max float64) float64 {
	...
	nvalue := float64(0.0)
	if matches := duration1.FindAllStringSubmatch(strings.ToUpper(value), -1); matches != nil {
